...
def take_screenshot_and_extract_text():
    ...
    os.remove(screenshot_path)  # Delete the screenshot after extracting text
...
# Function to check if at least 8 characters match between two texts
def at_least_8_chars_match(text1, text2):
    match_count = 0
...
def process_line(line):
    ...
    if not cleaned_extracted_text:
        print(f"No characters extracted for {col0}. Proceeding with creation.")
    # Check if at least 8 characters match between cleaned extracted text and cleaned col0
    elif at_least_8_chars_match(cleaned_extracted_text, cleaned_col0):
        print(f"Commander {col0} orgset is already created Sir.")
        log_already_created(col0)
        return

    # Proceed with creation if the above conditions are not met
